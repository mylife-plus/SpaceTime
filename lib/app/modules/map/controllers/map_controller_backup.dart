...
//   // UI state management (keeping for UI components)
...
//   Map<int, Color> getYearColorMappings({int startYear = -50, int endYear = 50}) {
...
//   List<int> getYearsForColor(Color color, {int startYear = -50, int endYear = 50}) {
...
//       debugPrint('🗺 OFFLINE - Components initialized: true, Downloaded tiles: $hasDownloadedTiles');
...
//       if (tileStore == null || offlineManager == null) {
//         debugPrint('🗺 OFFLINE - Offline components not initialized, not in offline mode');
//         return false;
//       }
...
//         debugPrint('🗺 OFFLINE - No downloaded tiles found, not in offline mode');
//         return false;
//       }
...
//       debugPrint('🗺 OFFLINE - Maps offline mode status: $isUsingOfflineResources');
...
//       debugPrint('🗺 OFFLINE - Total tiles downloaded: $tileCount (threshold: 50,000)');

//       // Return true only if we have 50,000+ tiles for reliable offline mode
//       return tileCount >= MAX_TILES_COUNT;
...
//         'usagePercentage': tileCount > 0 ? (tileCount / maxTiles * 100).round() : 0,
//         'forceOfflineMode': forceOffline,
//         'isCurrentlyDownloading': isDownloading,
//         'offlineComponentsInitialized': tileStore != null && offlineManager != null,
//       };
...
//         debugPrint('🗺 OFFLINE - Offline mode status changed: $wasOffline -> $nowOffline');
...
//   final RxList<mapbox.Position> locations =
//       <mapbox.Position>[
//       ].obs;
...
//   void onInit() {
//     super.onInit();
//     debugPrint('[MapController][onInit] Starting refined flow initialization');
...
//     // Start the new refined initialization flow
//     _startNewRefinedFlow();

//     debugPrint('[MapController][onInit] Refined flow initialization complete');
//   }

//   // Initialize offline functionality in background without blocking UI
//   Future<void> _initializeOfflineInBackground() async {
//     try {
//       debugPrint('[MapController][_initializeOfflineInBackground] Starting background offline initialization');
//       debugPrint('[MapController][_initializeOfflineInBackground] offlineManager null: ${offlineManager == null}');
//       debugPrint('[MapController][_initializeOfflineInBackground] tileStore null: ${tileStore == null}');
//       await initOfflineMap();
//       debugPrint('[MapController][_initializeOfflineInBackground] Background offline initialization completed');
//       debugPrint('[MapController][_initializeOfflineInBackground] offlineManager null after init: ${offlineManager == null}');
//       debugPrint('[MapController][_initializeOfflineInBackground] tileStore null after init: ${tileStore == null}');
//     } catch (e) {
//       debugPrint('[MapController][_initializeOfflineInBackground] Background offline initialization failed: $e');
//       debugPrint('[MapController][_initializeOfflineInBackground] Error type: ${e.runtimeType}');
//       debugPrint('[MapController][_initializeOfflineInBackground] Stack trace: ${StackTrace.current}');
//     }
//   }
...
//           debugPrint('[MapController] Background tile download service initialized successfully');
//         } catch (importError) {
//           debugPrint('[MapController] Error importing background service: $importError');
//         }
//       } else {
//         debugPrint('[MapController] TileStore or OfflineManager not available for background service');
//       }
//     } catch (e) {
//       debugPrint('[MapController] Error initializing background tile service: $e');
//     }
//   }
...
//       // State management removed - keeping UI reactive listeners

//       debugPrint('🔧 Sequential service listeners set up successfully');
//     } catch (e) {
//       debugPrint('❌ Error setting up service listeners: $e');
//     }
//   }

//   /// Handle permission state changes - simplified without state machine
//   void _handlePermissionStateChange(bool hasPermission) {
//     debugPrint('🔐 [PERMISSION CHANGE] Permission state changed: $hasPermission');

//     if (hasPermission) {
//       debugPrint('🔐 [PERMISSION CHANGE] ✅ Permission granted - refreshing map');
//       // Simply refresh the map when permission is granted
//       onInit();
//       refreshMapView();
//     } else {
//       debugPrint('🔐 [PERMISSION CHANGE] ❌ Permission denied');
//       // UI will handle showing permission screen based on permission service state
//     }
//   }

//   /// Handle connectivity state changes - simplified without state machine
//   void _handleConnectivityStateChange(bool isConnected) {
//     debugPrint('🌐 Connectivity state changed: $isConnected');

//     if (isConnected) {
//       debugPrint('🌐 ✅ Internet connectivity restored - refreshing map');
//       // Simply refresh the map when connectivity is restored
//       Future.microtask(() async {
//         await automaticMapReloadAfterConnectivityRestore();
//       });
//     } else {
//       debugPrint('🌐 ❌ Lost internet connectivity');
//       // UI will handle showing internet screen based on connectivity service state
//     }
//   }
...
//         // Check tile count - show internet widget if < 50,000 tiles
//         final backgroundService = Get.find<BackgroundTileDownloadService>();
//         final tileCount = backgroundService.totalTilesDownloaded.value;
//         final hasSufficientTiles = tileCount >= MAX_TILES_COUNT;

//         debugPrint('🌐 No internet access - tile count: $tileCount, sufficient: $hasSufficientTiles');

//         if (!hasSufficientTiles) {
//           debugPrint('🌐 No internet and insufficient tiles (< 50,000) - setting internetRequired state');
//           _setState(MapInitializationState.internetRequired);
//         } else {
//           debugPrint('🌐 No internet but sufficient tiles (≥ 50,000) - staying in ready state');
//         }
//       } else {
//         debugPrint('🌐 Internet access available - no need for internet screen');
//       }
//     } catch (e) {
...
//       debugPrint('🗺 Confirmed Mapbox connectivity error - checking current state');
...
//         debugPrint('🗺 Map was ready but Mapbox reports connectivity issues - checking internet');
//         _checkIfInternetScreenNeeded();
//       } else if (currentInitializationState.value == MapInitializationState.loadingMap ||
//                  currentInitializationState.value == MapInitializationState.downloadingTiles) {
//         debugPrint('🗺 Mapbox connectivity error during loading - setting internetRequired state');
//         _setState(MapInitializationState.internetRequired);
//       }
...
//   // State management methods removed - keeping UI reactive

//   // State logging methods removed

//   // State advancement methods removed
...
//       final hasInternetForMapbox = await connectivityService.hasInternetQuickCheck();
...
//         debugPrint('🌐 No internet for Mapbox detected, checking offline tiles');
...
//         debugPrint('🌐 Internet check failed with connectivity error - setting internetRequired state');
//         _setState(MapInitializationState.internetRequired);
//       } else {
...
//     try {
//       debugPrint('🗺 Checking offline tiles availability...');
//       final hasOfflineTiles = await isOfflineDataAvailable();
//       debugPrint('🗺 Offline tiles available: $hasOfflineTiles');

//       if (hasOfflineTiles) {
//         // We have offline tiles, but still need to check permissions first
//         debugPrint('🗺 Offline tiles found - but checking permissions first');

//         // Check if we're in initial states where permissions haven't been checked
//         if (currentInitializationState.value == MapInitializationState.initial ||
//             currentInitializationState.value == MapInitializationState.checkingPermission) {
//           _setState(MapInitializationState.checkingPermission);
//         } else {
//           // Permissions already handled, proceed to load map
//           _setState(MapInitializationState.loadingMap);
//         }
//       } else {
//         // No offline tiles and no internet - but only show internet screen if permissions are granted
//         debugPrint('🌐 No offline tiles and no internet - checking permission state first');

//         final permissionService = Get.find<PermissionService>();
//         if (permissionService.hasLocationPermission.value) {
//           // Permissions granted, can show internet screen
//           debugPrint('🌐 Permissions granted - setting internetRequired state');
//           _setState(MapInitializationState.internetRequired);
//         } else {
//           // Permissions not granted, check permissions first
//           debugPrint('🔐 Permissions not granted - checking permissions first');
//           _setState(MapInitializationState.checkingPermission);
//         }
//       }
//     } catch (e) {
...
//   // State processing methods removed - keeping UI reactive

//   // Permission check methods removed - UI handles permission directly
...
//       debugPrint('🗺 Tile download process completed');
//     } catch (e) {
//       debugPrint('❌ Error downloading tiles: $e');
//     }
//   }

//   /// Start map loading process - simplified without state management
...
//       debugPrint('🗺 Map loading completed');
//     } catch (e) {
...
//   /// Start the new refined initialization flow
//   Future<void> _startNewRefinedFlow() async {
//     try {
//       debugPrint('🚀 Starting new refined initialization flow');

//       // Set initial state
//       _setState(MapInitializationState.initial);

//       // Initialize offline functionality in background (non-blocking)
//       Future.microtask(() async {
//         await _initializeOfflineInBackground();
//       });

//       // Step 1: Check Location Permission first
//       await _checkLocationPermissionStep();

//       debugPrint('🚀 New refined initialization flow started successfully');
//     } catch (e) {
//       debugPrint('❌ Error starting new refined initialization flow: $e');
//       _setState(MapInitializationState.error);
//     }
//   }

//   /// Step 1: Check Location Permission
//   Future<void> _checkLocationPermissionStep() async {
//     try {
//       debugPrint('🔐 Step 1: Checking location permission');
//       _setState(MapInitializationState.checkingPermission);
...
//       if (hasPermission) {
//         debugPrint('🔐 Location permission granted - proceeding to load map');
//         // Step 2: Load Map View (this will trigger onMapCreated)
//         _setState(MapInitializationState.loadingMap);
//       } else {
//         debugPrint('🔐 Location permission denied - showing permission screen');
...
//   /// Refined Step 1: Check Location Permission
//   Future<void> _checkLocationPermissionRefined() async {
//     debugPrint('🔐 Refined Flow - Step 1: Checking location permission');

//     try {
//       final permissionService = Get.find<PermissionService>();
//       final hasPermission = await permissionService.checkLocationPermission(requestIfDenied: false);

//       if (hasPermission) {
//         debugPrint('🔐 Permission already granted, proceeding to map loading');
//         await _loadMapViewRefined();
//       } else {
//         debugPrint('🔐 Permission not granted, showing permission screen');
//         _setState(MapInitializationState.permissionDenied);
//       }
//     } catch (e) {
//       debugPrint('❌ Error checking location permission: $e');
...
//   /// Refined Step 2: Load Map View (after permission granted)
//   Future<void> _loadMapViewRefined() async {
//     debugPrint('🗺 Refined Flow - Step 2: Loading map view');

//     try {
//       _setState(MapInitializationState.loadingMap);

//       // Initialize map with Mapbox event listeners
//       await _initializeMapWithEventListeners();

//     } catch (e) {
//       debugPrint('❌ Error loading map view: $e');
//       await _handleMapboxErrorRefined(e);
//     }
//   }

//   /// Initialize map with Mapbox event listeners for refined flow
//   Future<void> _initializeMapWithEventListeners() async {
//     debugPrint('🗺 Setting up Mapbox with event listeners');

//     try {
//       // Perform the actual map initialization
//       await _performDirectMapInitialization();

//       // Set up Mapbox event listeners
//       _setupMapboxEventListeners();

//     } catch (e) {
//       debugPrint('❌ Error in map initialization with listeners: $e');
//       throw e;
//     }
//   }

//   /// Set up Mapbox event listeners for the refined flow
//   void _setupMapboxEventListeners() {
//     debugPrint('🎧 Setting up Mapbox event listeners');

//     // Note: Event listeners are set up in the MapWidget's onMapLoadErrorListener
//     // This method is called after successful map initialization
//     debugPrint('✅ Map loaded successfully - proceeding normally');
//     _setState(MapInitializationState.ready);
//     refreshMapView();
//   }

//   /// Handle Mapbox errors in refined flow
//   Future<void> _handleMapboxErrorRefined(dynamic error) async {
//     debugPrint('🔍 Refined Flow - Handling Mapbox error: $error');

//     try {
//       // Check if we have offline tiles available
//       final backgroundService = Get.find<BackgroundTileDownloadService>();
//       final hasOfflineTiles = backgroundService.totalTilesDownloaded.value > 1000;

//       if (hasOfflineTiles) {
//         debugPrint('📱 Offline tiles available, switching to offline mode');
//         isOfflineMode.value = true;
//         _setState(MapInitializationState.ready);
//         refreshMapView();
//       } else {
//         debugPrint('🌐 No offline tiles, showing Internet Required Screen');
//         _setState(MapInitializationState.internetRequired);
//         _startConnectivityMonitoring();
//       }
//     } catch (e) {
//       debugPrint('❌ Error handling Mapbox error: $e');
//       _setState(MapInitializationState.error);
//     }
//   }
...
//     debugPrint('🔄 Starting background connectivity monitoring');

//     final connectivityService = Get.find<ConnectivityService>();

//     // Listen for connectivity restoration
//     ever(connectivityService.isConnected, (bool isConnected) async {
//       if (isConnected && currentInitializationState.value == MapInitializationState.internetRequired) {
//         debugPrint('🌐 Connectivity restored, reloading map automatically');
//         await _reloadMapAfterConnectivityRestore();
//       }
//     });
//   }

//   /// Reload map automatically after connectivity is restored
//   Future<void> _reloadMapAfterConnectivityRestore() async {
//     debugPrint('🔄 Reloading map after connectivity restore');

//     try {
//       // Hide internet required screen
//       _setState(MapInitializationState.loadingMap);

//       // Reload the map
//       await _loadMapViewRefined();

//       // Trigger tile downloading if needed
//       final backgroundService = Get.find<BackgroundTileDownloadService>();
//       if (backgroundService.totalTilesDownloaded.value < 10000) {
//         debugPrint('📥 Triggering tile download after connectivity restore');
//         // Use the existing method to resume downloads
//         await backgroundService.setAutoDownloadEnabled(true);
//       }

//     } catch (e) {
//       debugPrint('❌ Error reloading map after connectivity restore: $e');
//       _setState(MapInitializationState.internetRequired);
//     }
//   }

//   /// Start the sequential initialization process (legacy method for compatibility)
//   Future<void> startSequentialInitialization() async {
//     debugPrint('🚀 Starting sequential initialization process (legacy)');
//     await _startNewRefinedFlow();
//   }

//   /// Retry from current state (for user-triggered retries)
//   Future<void> retryCurrentState() async {
//     debugPrint('🔄 Retrying current state: ${currentInitializationState.value}');

//     try {
//       switch (currentInitializationState.value) {
//         case MapInitializationState.permissionDenied:
//           debugPrint('🔄 [RETRY] Retrying from permission denied state');
//           _setState(
//             MapInitializationState.checkingPermission,
//             reason: 'User-triggered retry from permission denied state',
//             context: {
//               'retryAction': 'permission_check',
//               'previousState': 'permissionDenied'
//             }
//           );
//           break;
...
//         default:
//           debugPrint('🔄 Retrying from state: ${currentInitializationState.value}');
...
//       debugPrint('🌐 Force refresh results: isConnected=$isConnected, type=$connectionType');

//       // If we're in internetRequired state and now have internet, advance
//       if (currentInitializationState.value == MapInitializationState.internetRequired && isConnected) {
//         debugPrint('🌐 Internet restored during force refresh - advancing state');
//         _setState(MapInitializationState.checkingInternet);
//       }
...
//         debugPrint('🌐 Map controller is null - triggering map recreation');

//         // Reset map ready state to trigger recreation
...
//         // The MapView widget will automatically recreate the map when isMapReady becomes false
//         // and then true again, which will call onMapCreated
//         isMapReady.value = true;

//         debugPrint('🌐 Map recreation triggered - onMapCreated will be called automatically');
...
//             CameraOptions(
//               center: Point(coordinates: mapbox.Position(position.longitude, position.latitude)),
//               zoom: 12.0,
//             ),
...
//     debugPrint('[MapController][_checkPermissionsAndScheduleOneTimeRecreation] Starting permission check');
//     debugPrint('[MapController][_checkPermissionsAndScheduleOneTimeRecreation] Current thread: ${DateTime.now().millisecondsSinceEpoch}');

//     // Check location permissions
//     debugPrint('[MapController][_checkPermissionsAndScheduleOneTimeRecreation] Checking location permission...');
//     final hasLocationPermission = await _checkLocationPermission();
//     debugPrint('[MapController][_checkPermissionsAndScheduleOneTimeRecreation] Location permission result: $hasLocationPermission');

//     debugPrint('[MapController][_checkPermissionsAndScheduleOneTimeRecreation] Scheduling one-time recreation');
//     _scheduleOneTimeRecreation();
//     debugPrint('[MapController][_checkPermissionsAndScheduleOneTimeRecreation] Permission check completed');
//   }

//   void _scheduleOneTimeRecreation() {
//     debugPrint('[MapController][_scheduleOneTimeRecreation] Starting recreation scheduling');
//     debugPrint('[MapController][_scheduleOneTimeRecreation] _hasTriggeredRecreation: $_hasTriggeredRecreation');
//     debugPrint('[MapController][_scheduleOneTimeRecreation] _oneTimeRecreationTimer null: ${_oneTimeRecreationTimer == null}');

//     if (_hasTriggeredRecreation) {
//       debugPrint('[MapController][_scheduleOneTimeRecreation] Already triggered, skipping');
//       return;
//     }

//     debugPrint('[MapController][_scheduleOneTimeRecreation] Scheduling recreation in 100ms');
//     debugPrint('[MapController][_scheduleOneTimeRecreation] Current time: ${DateTime.now().millisecondsSinceEpoch}');
...
//     _mapRecreationCount++;
//     debugPrint('[MapController][_triggerMapRecreation] Starting recreation #$_mapRecreationCount');
//     debugPrint('[MapController][_triggerMapRecreation] Current state before recreation:');
//     debugPrint('[MapController][_triggerMapRecreation]   - mapController null: ${mapController == null}');
//     debugPrint('[MapController][_triggerMapRecreation]   - currentAnnotationManager null: ${currentAnnotationManager == null}');
//     debugPrint('[MapController][_triggerMapRecreation]   - annotations count: ${annotations.length}');
//     debugPrint('[MapController][_triggerMapRecreation]   - hasInitialized: ${hasInitialized.value}');
//     debugPrint('[MapController][_triggerMapRecreation]   - isMapReady: ${isMapReady.value}');
//     debugPrint('[MapController][_triggerMapRecreation]   - shouldRecreateMap: ${shouldRecreateMap.value}');
...
//     debugPrint('[MapController][_triggerMapRecreation] shouldRecreateMap toggled from $previousValue to ${shouldRecreateMap.value}');
...
//     debugPrint('[MapController][_triggerMapRecreation]   - hasInitialized: ${hasInitialized.value}');
//     debugPrint('[MapController][_triggerMapRecreation]   - isMapReady: ${isMapReady.value}');
//     debugPrint('[MapController][_triggerMapRecreation] Map state reset complete, MapWidget will rebuild');
//   }

//   Future<void> _setupReactiveWorkers() async {

//     debugPrint('[MapController][_setupReactiveWorkers] Starting reactive workers setup');
//     debugPrint('[MapController][_setupReactiveWorkers] Current state:');
//     debugPrint('[MapController][_setupReactiveWorkers]   - currentZoom: ${currentZoom.value}');
//     debugPrint('[MapController][_setupReactiveWorkers]   - isMapReady: ${isMapReady.value}');
//     debugPrint('[MapController][_setupReactiveWorkers]   - isShowingNewLocations: ${isShowingNewLocations.value}');
//     debugPrint('[MapController][_setupReactiveWorkers]   - locations count: ${locations.length}');

//     // Worker to handle zoom changes
//     debugPrint('[MapController][_setupReactiveWorkers] Setting up zoom change worker');
//     ever(currentZoom, (double zoom) {
//       debugPrint('[MapController][_setupReactiveWorkers][zoomWorker] Zoom change triggered: $zoom');
//       debugPrint('[MapController][_setupReactiveWorkers][zoomWorker] isMapReady: ${isMapReady.value}');
//       debugPrint('[MapController][_setupReactiveWorkers][zoomWorker] mapController null: ${mapController == null}');
//       debugPrint('[MapController][_setupReactiveWorkers][zoomWorker] _isTransitioningLocations: $_isTransitioningLocations');

//       if (isMapReady.value && mapController != null && !_isTransitioningLocations) {
//         debugPrint('[MapController][_setupReactiveWorkers][zoomWorker] Calling _ensureCameraZoom()');
//         _ensureCameraZoom();
//       } else {
//         debugPrint('[MapController][_setupReactiveWorkers][zoomWorker] Skipping zoom change - conditions not met');
//       }
//     });

//     // Worker to handle location state changes
//     debugPrint('[MapController][_setupReactiveWorkers] Setting up location state worker');
//     ever(isShowingNewLocations, (bool showingNew) {
//       debugPrint('[MapController][_setupReactiveWorkers][locationStateWorker] Location state change: $showingNew');
//       debugPrint('[MapController][_setupReactiveWorkers][locationStateWorker] This should trigger MapWidget rebuild via Obx');
//     });

//     // Worker to handle map ready state changes
//     debugPrint('[MapController][_setupReactiveWorkers] Setting up map ready state worker');
//     ever(isMapReady, (bool ready) {
//       debugPrint('[MapController][_setupReactiveWorkers][mapReadyWorker] Map ready state change: $ready');
//       debugPrint('[MapController][_setupReactiveWorkers][mapReadyWorker] mapController null: ${mapController == null}');
//       debugPrint('[MapController][_setupReactiveWorkers][mapReadyWorker] _isTransitioningLocations: $_isTransitioningLocations');

//       if (ready && mapController != null && !_isTransitioningLocations) {
//         debugPrint('[MapController][_setupReactiveWorkers][mapReadyWorker] Calling _ensureCameraZoom()');
//         _ensureCameraZoom();
//       } else {
//         debugPrint('[MapController][_setupReactiveWorkers][mapReadyWorker] Skipping - transitioning locations or controller null');
//       }
//     });

//     // Worker to handle locations changes
//     debugPrint('[MapController][_setupReactiveWorkers] Setting up locations change worker');
//     ever(locations, (List<mapbox.Position> newLocations) {
//       debugPrint('[MapController][_setupReactiveWorkers][locationsWorker] Locations change: ${newLocations.length} locations');
//       debugPrint('[MapController][_setupReactiveWorkers][locationsWorker] _isTransitioningLocations: $_isTransitioningLocations');
//       debugPrint('[MapController][_setupReactiveWorkers][locationsWorker] This should trigger MapWidget rebuild via Obx');
//     });

//     debugPrint('[MapController][_setupReactiveWorkers] All reactive workers setup completed');
//   }
...
//     debugPrint('[MapController][resetToOriginalLocations] Starting reset to original locations');
//     debugPrint('[MapController][resetToOriginalLocations] Current state before reset:');
//     debugPrint('[MapController][resetToOriginalLocations]   - _isTransitioningLocations: $_isTransitioningLocations');
//     debugPrint('[MapController][resetToOriginalLocations]   - currentAnnotationManager null: ${currentAnnotationManager == null}');
//     debugPrint('[MapController][resetToOriginalLocations]   - annotations count: ${annotations.length}');
//     debugPrint('[MapController][resetToOriginalLocations]   - locations count: ${locations.length}');
//     debugPrint('[MapController][resetToOriginalLocations]   - currentZoom: ${currentZoom.value}');
//     debugPrint('[MapController][resetToOriginalLocations]   - isMapReady: ${isMapReady.value}');
//     debugPrint('[MapController][resetToOriginalLocations]   - mapController null: ${mapController == null}');

//     // Set transition flag to prevent reactive zoom conflicts
//     debugPrint('[MapController][resetToOriginalLocations] Setting transition flag to true');
//     _isTransitioningLocations = true;
//     debugPrint('[MapController][resetToOriginalLocations] _isTransitioningLocations set to: $_isTransitioningLocations');

//     try {
//       // Clear existing annotations and lines first
//       debugPrint('[MapController][resetToOriginalLocations] Starting cleanup of existing annotations and lines');

//       if (currentAnnotationManager != null) {
//         debugPrint('[MapController][resetToOriginalLocations] Deleting all annotations from manager');
//         await currentAnnotationManager!.deleteAll();
//         debugPrint('[MapController][resetToOriginalLocations] All annotations deleted, setting manager to null');
//         currentAnnotationManager = null;
//       } else {
//         debugPrint('[MapController][resetToOriginalLocations] No annotation manager to clean up');
//       }

//       debugPrint('[MapController][resetToOriginalLocations] Clearing annotations list');
//       annotations.clear();
//       debugPrint('[MapController][resetToOriginalLocations] Clearing marker images');
...
//       debugPrint('[MapController][resetToOriginalLocations] Checking if map is ready for camera update');
//       debugPrint('[MapController][resetToOriginalLocations] isMapReady: ${isMapReady.value}');
//       debugPrint('[MapController][resetToOriginalLocations] mapController null: ${mapController == null}');
...
//         debugPrint('[MapController][resetToOriginalLocations] Map is ready, starting camera operations');
//         debugPrint('[MapController][resetToOriginalLocations] Target zoom: $targetZoom');
//         debugPrint('[MapController][resetToOriginalLocations] Target location: ${locations.isNotEmpty ? locations[0] : "NO LOCATIONS"}');

//         if (locations.isEmpty) {
//           debugPrint('[MapController][resetToOriginalLocations] ERROR: No locations available for camera positioning');
//           return;
//         }

//         // First ensure camera is at correct zoom and position with iOS-specific handling
//         debugPrint('[MapController][resetToOriginalLocations] Starting flyTo animation...');
//         debugPrint('[MapController][resetToOriginalLocations] Animation duration: 1500ms');
...
//         );
//         debugPrint('[MapController][resetToOriginalLocations] flyTo animation completed');

//         // Wait for the camera animation to complete
//         debugPrint('[MapController][resetToOriginalLocations] Waiting 1600ms for animation to settle');
//         await Future.delayed(const Duration(milliseconds: 1600));
//         debugPrint('[MapController][resetToOriginalLocations] Animation settle delay completed');

//         // Force another camera update for iOS reliability
//         debugPrint('[MapController][resetToOriginalLocations] Applying iOS reliability camera update');
...
//           ),
//         );
//         debugPrint('[MapController][resetToOriginalLocations] iOS reliability camera update completed');
...
//     try {
//       debugPrint('[MapController][loadMemoriesFromDatabase] Starting memory loading from database');
//       debugPrint('[MapController][loadMemoriesFromDatabase] isLoadingMemories before: ${isLoadingMemories.value}');
//       debugPrint('[MapController][loadMemoriesFromDatabase] allMemories count before: ${allMemories.length}');

//       isLoadingMemories.value = true;
//       debugPrint('[MapController][loadMemoriesFromDatabase] Set isLoadingMemories to true');

//       debugPrint('[MapController][loadMemoriesFromDatabase] Calling _databaseHelper.getAllMemoriesWithDetails()');
//       final memories = await _databaseHelper.getAllMemoriesWithDetails();
//       debugPrint('[MapController][loadMemoriesFromDatabase] Database query completed, got ${memories.length} memories');

//       allMemories.assignAll(memories);
//       debugPrint('[MapController][loadMemoriesFromDatabase] Assigned memories to allMemories, count: ${allMemories.length}');

//       if (memories.isEmpty) {
//         debugPrint('[MapController][loadMemoriesFromDatabase] No memories found in database, returning early');
...
//         debugPrint('[MapController][loadMemoriesFromDatabase][Current Position] $currentPosition');

//             await mapController!.flyTo(
//               mapbox.CameraOptions(
//                 center: mapbox.Point(coordinates: mapbox.Position(currentPosition?.longitude ?? 33.3, currentPosition?.latitude ?? 73.03)),
//                 zoom: 8.0,
//               ),
...
//         } catch (e) {
//           // debugPrint('[MapController][loadMemoriesFromDatabase] Error getting current location: $e');
//           // // Fallback to world view
//           // await mapController!.flyTo(
//           //   mapbox.CameraOptions(
//           //     center: mapbox.Point(coordinates: mapbox.Position0, 0)),
//           //     zoom: 2.0,
//           //   ),
//           //   mapbox.MapAnimationOptions(duration: 1000),
//           // );
...
//       debugPrint('[MapController][loadMemoriesFromDatabase] Debugging memory locations...');
//       MemoryClusteringService.debugMemoryLocations(memories);

//       // Initialize clustering with loaded memories
//       debugPrint('[MapController][loadMemoriesFromDatabase] Starting clustering initialization...');
//       await _initializeMemoryClustering();
//       debugPrint('[MapController][loadMemoriesFromDatabase] Clustering initialization completed');

//     } catch (e) {
//       debugPrint('[MapController][loadMemoriesFromDatabase] Error loading memories from database: $e');
//       debugPrint('[MapController][loadMemoriesFromDatabase] Error type: ${e.runtimeType}');
//       debugPrint('[MapController][loadMemoriesFromDatabase] Stack trace: ${StackTrace.current}');
//     } finally {
//       debugPrint('[MapController][loadMemoriesFromDatabase] Setting isLoadingMemories to false');
//       isLoadingMemories.value = false;
//       debugPrint('[MapController][loadMemoriesFromDatabase] loadMemoriesFromDatabase completed');
//     }
//   }

//   Future<void> _initializeMemoryClustering() async {
//     try {
//       debugPrint('[MapController][_initializeMemoryClustering] ========== STARTING CLUSTERING INITIALIZATION ==========');
//       debugPrint('[MapController][_initializeMemoryClustering] Total memories in allMemories: ${allMemories.length}');
//       debugPrint('[MapController][_initializeMemoryClustering] Current clusters count: ${currentClusters.length}');
//       debugPrint('[MapController][_initializeMemoryClustering] Current arrows count: ${currentArrows.length}');
//       debugPrint('[MapController][_initializeMemoryClustering] Map controller ready: ${mapController != null}');

//       // Reset cluster ID tracking to ensure fresh unique IDs
//       debugPrint('[MapController][_initializeMemoryClustering] Resetting cluster ID tracking for fresh start');
//       MemoryClusteringService.resetClusterIdTracking();

//       // Log all memories before filtering
//       debugPrint('[MapController][_initializeMemoryClustering] === ALL MEMORIES DATA ===');
...
//         debugPrint('[MapController][_initializeMemoryClustering]   - ID: ${memory['id']}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Title: ${memory['title'] ?? 'No title'}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Location: ${memory['location']}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Latitude: ${memory['latitude']}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Longitude: ${memory['longitude']}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Memory Date: ${memory['memory_date']}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Has valid coords: ${_hasValidCoordinates(memory)}');
//       }

//       // Convert memories to MemoryLocation objects, filtering out those without valid locations
//       debugPrint('[MapController][_initializeMemoryClustering] === FILTERING MEMORIES WITH VALID COORDINATES ===');
//       final memoriesWithCoordinates = allMemories
//           .where((memory) => _hasValidCoordinates(memory))
//           .toList();

//       debugPrint('[MapController][_initializeMemoryClustering] Filtered memories: ${memoriesWithCoordinates.length} out of ${allMemories.length} have valid coordinates');

//       // Log filtered memories
//       debugPrint('[MapController][_initializeMemoryClustering] === FILTERED MEMORIES DATA ===');
//       for (int i = 0; i < memoriesWithCoordinates.length; i++) {
//         final memory = memoriesWithCoordinates[i];
//         debugPrint('[MapController][_initializeMemoryClustering] Filtered Memory $i:');
//         debugPrint('[MapController][_initializeMemoryClustering]   - ID: ${memory['id']}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Location: ${memory['location']}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Lat/Lng: ${memory['latitude']}, ${memory['longitude']}');
//       }

//       debugPrint('[MapController][_initializeMemoryClustering] === CONVERTING TO MEMORY LOCATION OBJECTS ===');
//       final memoryLocations = memoriesWithCoordinates
//           .map((memory) => MemoryLocation.fromMap(memory))
//           .where((memoryLocation) =>
//               memoryLocation.latitude != 0.0 &&
//               memoryLocation.longitude != 0.0)
//           .toList();

//       debugPrint('[MapController][_initializeMemoryClustering] MemoryLocation objects created: ${memoryLocations.length}');

//       // Log MemoryLocation objects
//       debugPrint('[MapController][_initializeMemoryClustering] === MEMORY LOCATION OBJECTS ===');
//       for (int i = 0; i < memoryLocations.length; i++) {
//         final memLoc = memoryLocations[i];
//         debugPrint('[MapController][_initializeMemoryClustering] MemoryLocation $i:');
//         debugPrint('[MapController][_initializeMemoryClustering]   - ID: ${memLoc.id}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Title: ${memLoc.title}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Coordinates: ${memLoc.latitude}, ${memLoc.longitude}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Date: ${memLoc.memoryDate}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Metadata: ${memLoc.metadata}');
//       }

//       debugPrint('[MapController][_initializeMemoryClustering] === FINAL MEMORY LOCATIONS FOR CLUSTERING ===');
//       debugPrint('[MapController][_initializeMemoryClustering] Final memory locations count: ${memoryLocations.length}');

//       if (memoryLocations.isEmpty) {
//         debugPrint('[MapController][_initializeMemoryClustering] ❌ NO VALID MEMORIES FOR CLUSTERING');
//         debugPrint('[MapController][_initializeMemoryClustering] No memories with valid coordinates found for clustering');
//         debugPrint('[MapController][_initializeMemoryClustering] Clustering process aborted');
//         return;
//       }

//       // Use fixed 50km clustering radius as requested
//       double clusterRadius = MemoryClusteringService.cityClusterRadiusKm; // 50km
//       debugPrint('[MapController][_initializeMemoryClustering] === CLUSTERING CONFIGURATION ===');
//       debugPrint('[MapController][_initializeMemoryClustering] Cluster radius: ${clusterRadius}km');
//       debugPrint('[MapController][_initializeMemoryClustering] Memory locations to cluster: ${memoryLocations.length}');
//       debugPrint('[MapController][_initializeMemoryClustering] Clustering algorithm: MemoryClusteringService.clusterMemories');

//       // Initial clustering with adaptive radius
//       debugPrint('[MapController][_initializeMemoryClustering] === STARTING CLUSTERING PROCESS ===');
...
//       stopwatch.stop();
//       debugPrint('[MapController][_initializeMemoryClustering] === CLUSTERING COMPLETED ===');
//       debugPrint('[MapController][_initializeMemoryClustering] Clustering duration: ${stopwatch.elapsedMilliseconds}ms');
//       debugPrint('[MapController][_initializeMemoryClustering] Generated clusters: ${clusters.length}');

//       // Log detailed cluster information and verify ID uniqueness
//       debugPrint('[MapController][_initializeMemoryClustering] === CLUSTER DETAILS & ID VERIFICATION ===');
//       final Set<String> clusterIds = <String>{};
//       final List<String> duplicateIds = <String>[];

//       for (int i = 0; i < clusters.length; i++) {
//         final cluster = clusters[i];
//         debugPrint('[MapController][_initializeMemoryClustering] Cluster ${i + 1}:');
//         debugPrint('[MapController][_initializeMemoryClustering]   - ID: ${cluster.id}');

//         // Check for duplicate IDs
//         if (clusterIds.contains(cluster.id)) {
//           duplicateIds.add(cluster.id);
//           debugPrint('[MapController][_initializeMemoryClustering]   - ❌ DUPLICATE ID DETECTED: ${cluster.id}');
//         } else {
//           clusterIds.add(cluster.id);
//           debugPrint('[MapController][_initializeMemoryClustering]   - ✅ UNIQUE ID: ${cluster.id}');
//         }

//         debugPrint('[MapController][_initializeMemoryClustering]   - Memory count: ${cluster.memoryCount}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Center: ${cluster.centerLatitude.toStringAsFixed(6)}, ${cluster.centerLongitude.toStringAsFixed(6)}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Radius: ${cluster.radiusKm}km');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Earliest date: ${cluster.earliestDate}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Latest date: ${cluster.latestDate}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Is single memory: ${cluster.memoryCount == 1}');

//         // Log memories in this cluster
//         debugPrint('[MapController][_initializeMemoryClustering]   - Memories in cluster:');
//         for (int j = 0; j < cluster.memories.length; j++) {
//           final memory = cluster.memories[j];
//           debugPrint('[MapController][_initializeMemoryClustering]     Memory ${j + 1}: ${memory.id} - "${memory.title}" at ${memory.latitude}, ${memory.longitude}');
//         }
//       }

//       // Report ID uniqueness results
//       debugPrint('[MapController][_initializeMemoryClustering] === CLUSTER ID UNIQUENESS REPORT ===');
//       debugPrint('[MapController][_initializeMemoryClustering] Total clusters: ${clusters.length}');
//       debugPrint('[MapController][_initializeMemoryClustering] Unique cluster IDs: ${clusterIds.length}');
//       debugPrint('[MapController][_initializeMemoryClustering] Duplicate IDs found: ${duplicateIds.length}');

//       if (duplicateIds.isNotEmpty) {
//         debugPrint('[MapController][_initializeMemoryClustering] ❌ CRITICAL: Duplicate cluster IDs detected: $duplicateIds');
//       } else {
//         debugPrint('[MapController][_initializeMemoryClustering] ✅ SUCCESS: All cluster IDs are unique');
//       }

//       debugPrint('[MapController][_initializeMemoryClustering] === ASSIGNING CLUSTERS TO CONTROLLER ===');
//       debugPrint('[MapController][_initializeMemoryClustering] Previous currentClusters count: ${currentClusters.length}');
//       currentClusters.assignAll(clusters);
//       debugPrint('[MapController][_initializeMemoryClustering] New currentClusters count: ${currentClusters.length}');

//       // Generate chronological arrows with performance limits
//       debugPrint('[MapController][_initializeMemoryClustering] === GENERATING CHRONOLOGICAL ARROWS ===');
//       debugPrint('[MapController][_initializeMemoryClustering] Starting arrow generation for ${clusters.length} clusters');
//       debugPrint('[MapController][_initializeMemoryClustering] Previous currentArrows count: ${currentArrows.length}');

//       final arrowStopwatch = Stopwatch()..start();
//       final arrows = _generateOptimizedArrows(clusters);
//       arrowStopwatch.stop();

//       debugPrint('[MapController][_initializeMemoryClustering] Arrow generation completed in ${arrowStopwatch.elapsedMilliseconds}ms');
//       debugPrint('[MapController][_initializeMemoryClustering] Generated arrows count: ${arrows.length}');

//       currentArrows.assignAll(arrows);
//       debugPrint('[MapController][_initializeMemoryClustering] New currentArrows count: ${currentArrows.length}');

//       // Log arrow details
//       debugPrint('[MapController][_initializeMemoryClustering] === ARROW DETAILS ===');
//       for (int i = 0; i < arrows.length; i++) {
//         final arrow = arrows[i];
//         debugPrint('[MapController][_initializeMemoryClustering] Arrow ${i + 1}:');
//         debugPrint('[MapController][_initializeMemoryClustering]   - From: ${arrow.fromLatitude.toStringAsFixed(6)}, ${arrow.fromLongitude.toStringAsFixed(6)}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - To: ${arrow.toLatitude.toStringAsFixed(6)}, ${arrow.toLongitude.toStringAsFixed(6)}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - From Date: ${arrow.fromDate}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - To Date: ${arrow.toDate}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - From Cluster ID: ${arrow.fromClusterId}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - To Cluster ID: ${arrow.toClusterId}');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Distance: ${arrow.distance.toStringAsFixed(2)}km');
//         debugPrint('[MapController][_initializeMemoryClustering]   - Bearing: ${arrow.bearing.toStringAsFixed(2)}°');
//       }

//       debugPrint('[MapController][_initializeMemoryClustering] === SETTING CLUSTER LEVEL ===');
//       debugPrint('[MapController][_initializeMemoryClustering] Previous cluster level: ${currentClusterLevel.value}');
//       currentClusterLevel.value = ClusterLevel.initial;
//       debugPrint('[MapController][_initializeMemoryClustering] New cluster level: ${currentClusterLevel.value}');

//       debugPrint('[MapController][_initializeMemoryClustering] === CLUSTERING SUMMARY ===');
//       debugPrint('[MapController][_initializeMemoryClustering] Total memories processed: ${memoryLocations.length}');
//       debugPrint('[MapController][_initializeMemoryClustering] Clusters created: ${clusters.length}');
//       debugPrint('[MapController][_initializeMemoryClustering] Arrows generated: ${arrows.length}');
//       debugPrint('[MapController][_initializeMemoryClustering] Cluster level: ${currentClusterLevel.value}');

//       // Display clusters on map
//       debugPrint('[MapController][_initializeMemoryClustering] === STARTING MAP DISPLAY ===');
//       debugPrint('[MapController][_initializeMemoryClustering] Calling _displayMemoryClusters()');
//       await _displayMemoryClusters();
//       debugPrint('[MapController][_initializeMemoryClustering] Map display completed successfully');

//       debugPrint('[MapController][_initializeMemoryClustering] ========== CLUSTERING INITIALIZATION COMPLETED ==========');
...
//   List<ChronologicalArrow> _generateOptimizedArrows(List<MemoryCluster> clusters) {
//     debugPrint('[MapController][_generateOptimizedArrows] === STARTING ARROW GENERATION ===');
//     debugPrint('[MapController][_generateOptimizedArrows] Input clusters count: ${clusters.length}');

//     // Log cluster information for arrow generation
//     debugPrint('[MapController][_generateOptimizedArrows] === CLUSTERS FOR ARROW GENERATION ===');
//     for (int i = 0; i < clusters.length; i++) {
//       final cluster = clusters[i];
//       debugPrint('[MapController][_generateOptimizedArrows] Cluster ${i + 1} for arrows:');
//       debugPrint('[MapController][_generateOptimizedArrows]   - ID: ${cluster.id}');
//       debugPrint('[MapController][_generateOptimizedArrows]   - Memory count: ${cluster.memoryCount}');
//       debugPrint('[MapController][_generateOptimizedArrows]   - Center: ${cluster.centerLatitude}, ${cluster.centerLongitude}');
//       debugPrint('[MapController][_generateOptimizedArrows]   - Earliest date: ${cluster.earliestDate}');
//       debugPrint('[MapController][_generateOptimizedArrows]   - Latest date: ${cluster.latestDate}');
//     }

//     // For performance, limit arrow generation for large datasets
//     if (clusters.length > 50) {
//       debugPrint('[MapController][_generateOptimizedArrows] === PERFORMANCE OPTIMIZATION ===');
//       debugPrint('[MapController][_generateOptimizedArrows] Large dataset detected (${clusters.length} clusters)');
//       debugPrint('[MapController][_generateOptimizedArrows] Applying performance optimization - using top 30 clusters');
...
//       debugPrint('[MapController][_generateOptimizedArrows] Top clusters selected for arrow generation:');
//       for (int i = 0; i < topClusters.length; i++) {
//         final cluster = topClusters[i];
//         debugPrint('[MapController][_generateOptimizedArrows] Top cluster ${i + 1}: ${cluster.memoryCount} memories');
//       }

//       debugPrint('[MapController][_generateOptimizedArrows] Calling MemoryClusteringService.generateChronologicalArrows with ${topClusters.length} clusters');
//       final arrows = MemoryClusteringService.generateChronologicalArrows(topClusters);
//       debugPrint('[MapController][_generateOptimizedArrows] Generated ${arrows.length} arrows from optimized clusters');
//       return arrows;
//     } else {
//       debugPrint('[MapController][_generateOptimizedArrows] === STANDARD ARROW GENERATION ===');
//       debugPrint('[MapController][_generateOptimizedArrows] Normal dataset size (${clusters.length} clusters)');
//       debugPrint('[MapController][_generateOptimizedArrows] Calling MemoryClusteringService.generateChronologicalArrows with all clusters');

//       final arrows = MemoryClusteringService.generateChronologicalArrows(clusters);
//       debugPrint('[MapController][_generateOptimizedArrows] Generated ${arrows.length} arrows from all clusters');
//       return arrows;
//     }
//   }

//   bool _hasValidCoordinates(Map<String, dynamic> memory) {
//     debugPrint('[MapController][_hasValidCoordinates] === VALIDATING COORDINATES ===');
//     debugPrint('[MapController][_hasValidCoordinates] Memory ID: ${memory['id']}');

//     final locationStr = memory['location'] as String? ?? '';
//     debugPrint('[MapController][_hasValidCoordinates] Location string: "$locationStr"');

//     // Skip if location is empty or null
//     if (locationStr.isEmpty) {
//       debugPrint('[MapController][_hasValidCoordinates] ❌ INVALID: Location string is empty');
//       return false;
//     }

//     // Skip if location doesn't contain coordinates (comma-separated values)
//     if (!locationStr.contains(',')) {
//       debugPrint('[MapController][_hasValidCoordinates] ❌ INVALID: Location string does not contain comma separator');
//       return false;
//     }

//     final parts = locationStr.split(',');
//     debugPrint('[MapController][_hasValidCoordinates] Location parts: ${parts.length} parts - $parts');

//     if (parts.length < 2) {
//       debugPrint('[MapController][_hasValidCoordinates] ❌ INVALID: Less than 2 coordinate parts');
//       return false;
//     }

//     final latStr = parts[0].trim();
//     final lngStr = parts[1].trim();
//     debugPrint('[MapController][_hasValidCoordinates] Latitude string: "$latStr"');
//     debugPrint('[MapController][_hasValidCoordinates] Longitude string: "$lngStr"');
...
//       debugPrint('[MapController][_hasValidCoordinates] ❌ INVALID: Failed to parse coordinates as numbers');
//       return false;
//     }

//     // Skip if coordinates are exactly 0,0 (likely default/invalid)
//     if (lat == 0.0 && lng == 0.0) {
//       debugPrint('[MapController][_hasValidCoordinates] ❌ INVALID: Coordinates are 0,0 (likely default/invalid)');
//       return false;
//     }

//     // Skip if coordinates are outside valid ranges
//     if (lat < -90.0 || lat > 90.0 || lng < -180.0 || lng > 180.0) {
//       debugPrint('[MapController][_hasValidCoordinates] ❌ INVALID: Coordinates outside valid ranges');
//       debugPrint('[MapController][_hasValidCoordinates] Latitude range: -90 to 90, got: $lat');
//       debugPrint('[MapController][_hasValidCoordinates] Longitude range: -180 to 180, got: $lng');
//       return false;
//     }

//     debugPrint('[MapController][_hasValidCoordinates] ✅ VALID: Coordinates are valid ($lat, $lng)');
//     return true;
//   }

//   Future<void> _displayMemoryClusters({bool clearExisting = true}) async {
//     debugPrint('[MapController][_displayMemoryClusters] Starting display process...');
//     debugPrint('[MapController][_displayMemoryClusters] clearExisting: $clearExisting');
//     debugPrint('[MapController][_displayMemoryClusters] mapController null: ${mapController == null}');
//     debugPrint('[MapController][_displayMemoryClusters] currentClusters count: ${currentClusters.length}');
//     debugPrint('[MapController][_displayMemoryClusters] currentAnnotationManager null: ${currentAnnotationManager == null}');
//     debugPrint('[MapController][_displayMemoryClusters] annotations count: ${annotations.length}');
...
//     debugPrint('[MapController][_displayMemoryClusters] Starting retry loop with max $maxRetries retries');

//     while (retryCount < maxRetries) {
//       debugPrint('[MapController][_displayMemoryClusters] Retry attempt ${retryCount + 1}/$maxRetries');

//       if (mapController == null) {
//         debugPrint('[MapController][_displayMemoryClusters] Map controller is null, retry ${retryCount + 1}/$maxRetries');
//         retryCount++;
//         if (retryCount < maxRetries) {
//           debugPrint('[MapController][_displayMemoryClusters] Waiting ${retryDelay.inMilliseconds}ms before retry');
//           await Future.delayed(retryDelay);
//           continue;
//         } else {
//           debugPrint('[MapController][_displayMemoryClusters] CRITICAL: Map controller still null after $maxRetries retries, exiting');
//           return;
//         }
//       }

//       if (currentClusters.isEmpty) {
//         debugPrint('[MapController][_displayMemoryClusters] Current clusters empty, retry ${retryCount + 1}/$maxRetries');
//         retryCount++;
//         if (retryCount < maxRetries) {
//           debugPrint('[MapController][_displayMemoryClusters] Waiting ${retryDelay.inMilliseconds}ms before retry');
//           await Future.delayed(retryDelay);
//           continue;
//         } else {
//           debugPrint('[MapController][_displayMemoryClusters] CRITICAL: Clusters still empty after $maxRetries retries, exiting');
//           return;
//         }
//       }

//       // Both conditions satisfied, break out of retry loop
//       debugPrint('[MapController][_displayMemoryClusters] Retry conditions satisfied, proceeding with display');
//       break;
//     }

//     try {
//       debugPrint('[MapController][_displayMemoryClusters] Starting try block for cluster display');

//       // Only clear existing annotations if requested (for initial load or reset)
//       if (clearExisting) {
//         debugPrint('[MapController][_displayMemoryClusters] Clearing existing annotations...');
//         debugPrint('[MapController][_displayMemoryClusters] currentAnnotationManager null before clear: ${currentAnnotationManager == null}');
//         debugPrint('[MapController][_displayMemoryClusters] annotations count before clear: ${annotations.length}');

//         if (currentAnnotationManager != null) {
//           debugPrint('[MapController][_displayMemoryClusters] Calling deleteAll() on annotation manager');
//           await currentAnnotationManager!.deleteAll();
//           debugPrint('[MapController][_displayMemoryClusters] deleteAll() completed, setting manager to null');
//           currentAnnotationManager = null;
//         }

//         debugPrint('[MapController][_displayMemoryClusters] Clearing annotations list');
//         annotations.clear();
//         debugPrint('[MapController][_displayMemoryClusters] Clearing marker images');
//         await _clearAllMarkerImages();
//         debugPrint('[MapController][_displayMemoryClusters] Clearing lines');
//         await _clearAllLines();
//         debugPrint('[MapController][_displayMemoryClusters] All clearing operations completed');
//       } else {
//         debugPrint('[MapController][_displayMemoryClusters] Preserving existing annotations, adding new ones...');
//       }

//       // Create or reuse annotation manager
//       debugPrint('[MapController][_displayMemoryClusters] Checking annotation manager state');
//       if (currentAnnotationManager == null) {
//         debugPrint('[MapController][_displayMemoryClusters] Creating new annotation manager...');
//         debugPrint('[MapController][_displayMemoryClusters] mapController available: ${mapController != null}');
//         currentAnnotationManager = await mapController!.annotations.createPointAnnotationManager();
//         debugPrint('[MapController][_displayMemoryClusters] New annotation manager created: ${currentAnnotationManager != null}');
//       } else {
//         debugPrint('[MapController][_displayMemoryClusters] Reusing existing annotation manager');
//       }

//       debugPrint('[MapController][_displayMemoryClusters] Initializing marker options list');
//       final List<mapbox.PointAnnotationOptions> markerOptions = [];
//       debugPrint('[MapController][_displayMemoryClusters] Marker options list initialized, length: ${markerOptions.length}');

//       debugPrint('🔄 DISPLAY CLUSTERS - Creating ${currentClusters.length} cluster markers...');
...
//         debugPrint('[MapController][_displayMemoryClusters] === CREATING UNIQUE IMAGE FOR CLUSTER ===');
//         debugPrint('[MapController][_displayMemoryClusters] Image name: $imageName');
//         debugPrint('[MapController][_displayMemoryClusters] Cluster index: $i');

//         // Print all cluster data and its memories
//         debugPrint('[MapController][_displayMemoryClusters] === CLUSTER ${i + 1} COMPLETE DATA ===');
//         debugPrint('[MapController][_displayMemoryClusters] Cluster ID: ${cluster.id}');
//         debugPrint('[MapController][_displayMemoryClusters] Memory Count: ${cluster.memoryCount}');
//         debugPrint('[MapController][_displayMemoryClusters] Center Coordinates: ${cluster.centerLatitude.toStringAsFixed(6)}, ${cluster.centerLongitude.toStringAsFixed(6)}');
//         debugPrint('[MapController][_displayMemoryClusters] Radius: ${cluster.radiusKm}km');
//         debugPrint('[MapController][_displayMemoryClusters] Earliest Date: ${cluster.earliestDate}');
//         debugPrint('[MapController][_displayMemoryClusters] Latest Date: ${cluster.latestDate}');
//         debugPrint('[MapController][_displayMemoryClusters] Is Single Memory: ${cluster.memoryCount == 1}');

//         // Print all memories in this cluster
//         debugPrint('[MapController][_displayMemoryClusters] === MEMORIES IN CLUSTER ${i + 1} ===');
//         for (int memIndex = 0; memIndex < cluster.memories.length; memIndex++) {
//           final memory = cluster.memories[memIndex];
//           debugPrint('[MapController][_displayMemoryClusters] Memory ${memIndex + 1}:');
//           debugPrint('[MapController][_displayMemoryClusters]   - ID: ${memory.id}');
//           debugPrint('[MapController][_displayMemoryClusters]   - Title: "${memory.title}"');
//           debugPrint('[MapController][_displayMemoryClusters]   - Description: "${memory.description}"');
//           debugPrint('[MapController][_displayMemoryClusters]   - Coordinates: ${memory.latitude.toStringAsFixed(6)}, ${memory.longitude.toStringAsFixed(6)}');
//           debugPrint('[MapController][_displayMemoryClusters]   - Memory Date: ${memory.memoryDate}');
//           debugPrint('[MapController][_displayMemoryClusters]   - Metadata Keys: ${memory.metadata.keys.toList()}');

//           // Print specific metadata fields if they exist
//           if (memory.metadata.containsKey('location')) {
//             debugPrint('[MapController][_displayMemoryClusters]   - Original Location: "${memory.metadata['location']}"');
//           }
//           if (memory.metadata.containsKey('place_category')) {
//             debugPrint('[MapController][_displayMemoryClusters]   - Place Category: "${memory.metadata['place_category']}"');
//           }
//           if (memory.metadata.containsKey('hashtags')) {
//             debugPrint('[MapController][_displayMemoryClusters]   - Hashtags: "${memory.metadata['hashtags']}"');
//           }
//           if (memory.metadata.containsKey('images')) {
//             final images = memory.metadata['images'];
//             debugPrint('[MapController][_displayMemoryClusters]   - Has Images: ${images != null && images.toString().isNotEmpty}');
//           }
//           if (memory.metadata.containsKey('voice_notes')) {
//             final voiceNotes = memory.metadata['voice_notes'];
//             debugPrint('[MapController][_displayMemoryClusters]   - Has Voice Notes: ${voiceNotes != null && voiceNotes.toString().isNotEmpty}');
//           }
//           if (memory.metadata.containsKey('created_at')) {
//             debugPrint('[MapController][_displayMemoryClusters]   - Created At: "${memory.metadata['created_at']}"');
//           }
//           if (memory.metadata.containsKey('updated_at')) {
//             debugPrint('[MapController][_displayMemoryClusters]   - Updated At: "${memory.metadata['updated_at']}"');
//           }

//           // Print all metadata for complete visibility
//           debugPrint('[MapController][_displayMemoryClusters]   - Complete Metadata: ${memory.metadata}');
//         }

//         debugPrint('[MapController][_displayMemoryClusters] === END CLUSTER ${i + 1} DATA ===');
//         debugPrint('[MapController][_displayMemoryClusters] Creating marker ${i + 1} for cluster at ${cluster.centerLatitude}, ${cluster.centerLongitude}');

//         // try {
//           // Clear any existing image with this name first
//           try {
//             await mapController!.style.removeStyleImage(imageName);
//             debugPrint('[MapController][_displayMemoryClusters] Removed existing image: $imageName');
//           } catch (e) {
//             debugPrint('[MapController][_displayMemoryClusters] No existing image to remove: $imageName');
//           }

//           // Create marker image based on cluster
//           debugPrint('[MapController][_displayMemoryClusters] Creating unique image for cluster ${cluster.id}');
//           final imageBytes = await _createClusterMarkerImage(cluster, i);

//           debugPrint('[MapController][_displayMemoryClusters] Created image bytes: ${imageBytes.length} bytes for $imageName');
...
//             imageName,
//             1.0,
//             mapbox.MbxImage(data: imageBytes, width: imageSize, height: imageSize),
//             false,
//             [],
//             [],
//             null,
...
//           debugPrint('🔄 [MapController][_displayMemoryClusters] DISPLAY CLUSTERS - Successfully added style image: $imageName');
...
//           debugPrint('🔄 [MapController][_displayMemoryClusters] DISPLAY CLUSTERS - Added marker option ${i + 1}');
...
//       debugPrint('🔄 DISPLAY CLUSTERS - Creating ${markerOptions.length} markers...');
//       final created = await currentAnnotationManager!.createMulti(markerOptions);
//       final validAnnotations = created.where((a) => a != null).cast<mapbox.PointAnnotation>().toList();
//       annotations.assignAll(validAnnotations);

//       debugPrint('🔄 DISPLAY CLUSTERS - Created ${validAnnotations.length} valid annotations');
...
//       debugPrint('🔄 DISPLAY CLUSTERS - Display process completed successfully');
...
//         (cluster) =>
//           (annotation.geometry.coordinates.lng - cluster.centerLongitude).abs() < 0.0001 &&
//           (annotation.geometry.coordinates.lat - cluster.centerLatitude).abs() < 0.0001,
//       );
...
//         debugPrint('Drilling down to show individual markers for cluster with ${tappedCluster.memoryCount} memories');
...
//     try {
//       debugPrint('Drilling down to subgroup for cluster with ${selectedCluster.memoryCount} memories');
...
//       debugPrint('Drilled down to ${subclusters.length} subclusters with ${arrows.length} arrows');
//     } catch (e) {
...
//     final internalArrows = MemoryClusteringService.generateChronologicalArrows(subclusters);
...
//     try {
//       debugPrint('🔍 DRILL DOWN - Starting drill down for ${clusterMemories.length} memories');
...
//       final subClusters = await _createSubClusters(clusterMemories, radiusKm: 0.5); // 100 meters
...
//       debugPrint('🔍 DRILL DOWN - Generating arrows between ${subClusters.length} sub-clusters');
//       final subClusterArrows = _generateOptimizedArrows(subClusters);
//       currentArrows.clear();
//       currentArrows.addAll(subClusterArrows);
//       debugPrint('🔍 DRILL DOWN - Generated ${subClusterArrows.length} arrows between sub-clusters');
...
//       debugPrint('🔄 DISPLAY MARKERS ONLY - Exiting early due to null controller or empty clusters');
//       return;
//     }

//     try {
//       debugPrint('🔄 DISPLAY MARKERS ONLY - Starting marker display process...');

//       // Create annotation manager if needed
//       if (currentAnnotationManager == null) {
//         debugPrint('🔄 DISPLAY MARKERS ONLY - Creating new annotation manager...');
//         currentAnnotationManager = await mapController!.annotations.createPointAnnotationManager();
//       } else {
//         debugPrint('🔄 DISPLAY MARKERS ONLY - Reusing existing annotation manager...');
//       }
//       final List<mapbox.PointAnnotationOptions> markerOptions = [];

//       debugPrint('🔄 DISPLAY MARKERS ONLY - Creating ${currentClusters.length} cluster markers...');
...
//         debugPrint('🔄 DISPLAY MARKERS ONLY - Creating marker ${i + 1} for cluster at ${cluster.centerLatitude}, ${cluster.centerLongitude}');

//         try {
//           // Create marker image based on cluster
//           final imageBytes = await _createClusterMarkerImage(cluster, index);

//           debugPrint('🔄 DISPLAY MARKERS ONLY - Adding style image: $imageName (${imageBytes.length} bytes)');
...
//           debugPrint('🔄 DISPLAY MARKERS ONLY - Successfully added style image: $imageName');
...
//         } catch (e) {
//           debugPrint('❌ DISPLAY MARKERS ONLY - Error creating marker ${i + 1}: $e');
//           // Skip this marker and continue with the next one
//           continue;
//         }
//       }
...
//       // Create markers
//       debugPrint('🔄 DISPLAY MARKERS ONLY - Creating ${markerOptions.length} markers...');
//       final created = await currentAnnotationManager!.createMulti(markerOptions);
//       final validAnnotations = created.where((a) => a != null).cast<mapbox.PointAnnotation>().toList();
//       annotations.assignAll(validAnnotations);

//       debugPrint('🔄 DISPLAY MARKERS ONLY - Created ${validAnnotations.length} valid annotations');

//       // Set up click listeners for drill-down clusters (different behavior)
//       debugPrint('🔄 DISPLAY MARKERS ONLY - Setting up drill-down click listeners...');
//       currentAnnotationManager!.addOnPointAnnotationClickListener(
//         AnnotationClickListener((annotation) async {
...
//       debugPrint('🔄 DISPLAY MARKERS ONLY - Marker display process completed successfully');

//     } catch (e) {
//       debugPrint('❌ DISPLAY MARKERS ONLY - Error displaying memory markers: $e');
//       debugPrint('❌ DISPLAY MARKERS ONLY - Stack trace: ${StackTrace.current}');
//     }
//   }

//   Future<void> _onDrillDownMarkerTapped(mapbox.PointAnnotation annotation) async {
//     try {
//       // Find which cluster was tapped
//       final tappedCluster = currentClusters.firstWhereOrNull(
//         (cluster) =>
...
//       debugPrint('🎯 DRILL-DOWN TAP - Cluster found with ${tappedCluster.memoryCount} memories');
...
//         debugPrint('  📄 Description: ${memory.metadata['description'] ?? 'No description'}');
//         debugPrint('  📂 Category: ${memory.metadata['category'] ?? 'No category'}');
//         debugPrint('  🏷️ Tags: ${memory.metadata['tags'] ?? 'No tags'}');
//         debugPrint('  📱 Created At: ${memory.metadata['created_at'] ?? 'Unknown'}');
//         debugPrint('  🔄 Updated At: ${memory.metadata['updated_at'] ?? 'Unknown'}');
//         debugPrint('  🗺️ Location String: ${memory.metadata['location'] ?? 'No location string'}');
//         debugPrint('  📸 Has Images: ${memory.metadata['images']?.isNotEmpty ?? false}');
//         debugPrint('  🎵 Has Audio: ${memory.metadata['audio_path']?.isNotEmpty ?? false}');
...
//         debugPrint('  📍 Center Location: ${tappedCluster.centerLatitude}, ${tappedCluster.centerLongitude}');
//         debugPrint('  🆔 Cluster ID: ${tappedCluster.id}');
//         debugPrint('  📏 Radius: ${tappedCluster.radiusKm}km');
//         debugPrint('  📅 Date Range: ${tappedCluster.earliestDate} to ${tappedCluster.latestDate}');

//         debugPrint('  📋 Individual Memories in Cluster:');

//         // Convert cluster memories to the format expected by AddMemoriesController
//         final specificMemories = tappedCluster.memories.map((memoryLocation) => memoryLocation.metadata).toList();

//         // Show bottom panel with cluster memories
//         showLocationBottomPanel(Get.context!, tappedCluster, specificMemories: specificMemories);
...
//                 label: 'Location',
//                 value: '${memory.latitude.toStringAsFixed(6)}, ${memory.longitude.toStringAsFixed(6)}',
//                 color: Colors.red,
//               ),
//             ),
...
//   try {
//     debugPrint('🧹 CLEAR - Clearing all markers, clusters, lines, and arrows');
...
//     try {
//       debugPrint('🧹 CLEAR - Clearing markers and clusters (preserving arrows)');
...
//   Future<List<MemoryCluster>> _createSubClusters(List<MemoryLocation> memories, {required double radiusKm}) async {
//     debugPrint('🔄 SUB-CLUSTERING - Creating sub-clusters with ${radiusKm * 1000}m radius for ${memories.length} memories');
...
//           processed[j] = true;
//           debugPrint('🔄 SUB-CLUSTERING - Added memory to cluster (distance: ${(distance * 1000).toStringAsFixed(0)}m)');
//         }
//       }
...
//       debugPrint('🔄 SUB-CLUSTERING - Created sub-cluster ${clusters.length} with ${clusterMemories.length} memories at $centerLat, $centerLng');
//     }

//     debugPrint('🔄 SUB-CLUSTERING - Sub-clustering completed: ${clusters.length} clusters created');
//     return clusters;
//   }
...
//     if (allMemories.isNotEmpty && currentClusters.isEmpty && mapController != null) {
...
//       final memories = await _databaseHelper.getAllMemoriesWithDetails();
//       debugPrint('🔍 DATABASE DEBUG - Found ${memories.length} memories in database');

//       if (memories.isEmpty) {
//         debugPrint('🔍 DATABASE DEBUG - Database is empty. No memories to display.');
//         return;
//       }
...
//       debugPrint('🔍 DATABASE DEBUG - $validLocationCount out of ${memories.length} memories have valid coordinates');

//       if (validLocationCount == 0) {
//         debugPrint('🔍 DATABASE DEBUG - No memories have valid location coordinates!');
//         debugPrint('🔍 DATABASE DEBUG - To see markers on the map, add memories with location data.');
//         debugPrint('🔍 DATABASE DEBUG - Location format should be: "latitude,longitude" (e.g., "37.7749,-122.4194")');
//       }
...
//         debugPrint('🔄 DUMMY MARKERS - Using user location: $userLat, $userLng');
//       } catch (e) {
//         debugPrint('🔄 DUMMY MARKERS - Could not get user location, using default: $e');
//       }
...
//         final imageBytes = await _createTestMarkerImage(location['count'] as int, i == 0);
...
//       debugPrint('🔄 DUMMY MARKERS - Created ${testLocations.length} test markers');
...
//   Future<Uint8List> _createTestMarkerImage(int count, bool isUserLocation) async {
...
//     // Choose color
//     final Color markerColor = isUserLocation
...
// Future<Uint8List> _createSingleMemoryMarker(MemoryCluster cluster, int clusterIndex, double size) async {
//   final radius = size / 2;
//   final recorder = PictureRecorder();
//   final canvas = Canvas(recorder);

//   final markerColor = _getUniqueColorForCluster(cluster.id, cluster.memories[0].id);
...
//   // Draw circle
//   canvas.drawCircle(Offset(radius, radius), radius - 6, Paint()..color = markerColor);
...
//         fontSize: 14,
//         fontWeight: FontWeight.bold,
//         shadows: [Shadow(offset: Offset(0.5, 0.5), blurRadius: 1, color: Colors.black87)],
//       ),
//     ),
//     textDirection: TextDirection.ltr,
//     textAlign: TextAlign.center,
...
// Future<Uint8List> _createMultiMemoryMarker(MemoryCluster cluster, double size) async {
//   final radius = size / 2;
//   final recorder = PictureRecorder();
//   final canvas = Canvas(recorder);
...
//   // Count text
//   final fontSize = cluster.memoryCount > 99 ? 10.0 : cluster.memoryCount > 9 ? 12.0 : 14.0;
//   final textPainter = TextPainter(
//     text: TextSpan(
...
//         fontSize: fontSize,
//         fontWeight: FontWeight.bold,
//         shadows: const [Shadow(offset: Offset(0.5, 0.5), blurRadius: 1, color: Colors.black87)],
//       ),
//     ),
//     textDirection: TextDirection.ltr,
//     textAlign: TextAlign.center,
...
//   Future<Uint8List> _createClusterMarkerImage(MemoryCluster cluster, int clusterIndex) async {
//     try {
...
//       debugPrint('[MapController][_createClusterMarkerImage] === CREATING UNIQUE CLUSTER IMAGE ===');
//       debugPrint('[MapController][_createClusterMarkerImage] Cluster ID: ${cluster.id}');
//       debugPrint('[MapController][_createClusterMarkerImage] Cluster Index: $clusterIndex');
//       debugPrint('[MapController][_createClusterMarkerImage] Memory Count: ${cluster.memoryCount}');
//       debugPrint('[MapController][_createClusterMarkerImage] Is Single Memory: ${cluster.isSingleMemory}');
//       debugPrint('[MapController][_createClusterMarkerImage] Memories Length: ${cluster.memories.length}');
...
//         debugPrint('[MapController][_createClusterMarkerImage] Single memory color: ${markerColor.toString()}');
//       } else {
...
//         // }
//         debugPrint('[MapController][_createClusterMarkerImage] Multi-memory color: ${markerColor.toString()}');
//       }
...
//       final fontSize = cluster.memoryCount > 99 ? 10.0 :
//                       cluster.memoryCount > 9 ? 12.0 : 14.0;

//       final textPainter = TextPainter(
//         text: TextSpan(
//           text: cluster.isSingleMemory ? (1).toString() : cluster.memoryCount.toString(),
//           style: TextStyle(
//             color: Colors.white,
//             fontSize: fontSize,
//             fontWeight: FontWeight.bold,
...
//       debugPrint('✅ Created cluster marker image: ${imageBytes.length} bytes, ${imageSize}x${imageSize}px');
...
//       await _addArrowHeadOnCurve(points, currentColor); // heads are point annotations
//     }
//   } catch (e) {
//     debugPrint('Error displaying chronological arrows: $e');
//   }
...
//     try {
//       await mapController!.style.removeStyleImage(imgKey); // refresh if same color used before
//     } catch (_) {}
...
//     final idx = ((curvePoints.length - 1) * 0.8).clamp(1, curvePoints.length - 1).toInt();
...
//       pPrev.lat.toDouble(), pPrev.lng.toDouble(),
//       pNow.lat.toDouble(),  pNow.lng.toDouble(),
//     );

//     await currentAnnotationManager!.create(
//       mapbox.PointAnnotationOptions(
//         geometry: mapbox.Point(coordinates: mapbox.Position(pNow.lng, pNow.lat)),
//         iconImage: imgKey,
//         iconSize: 0.8,
//         iconRotate: bearing,
//         // iconAllowOverlap: true,
...
//   final x = math.cos(radLat1) * math.sin(radLat2) -
...
//   Future<void> _addArrowHead(ChronologicalArrow arrow, [int? arrowColor]) async {
...
//     final backgroundPaint = Paint()..color = Colors.transparent;
//     canvas.drawRect(Rect.fromLTWH(0, 0, size.toDouble(), size.toDouble()), backgroundPaint);
...
//     final imageBytes = byteData.buffer.asUint8List();
//     debugPrint('✅ Created arrow head image: ${imageBytes.length} bytes, ${size}x${size}px');
//     return imageBytes;
//   }
...
//       double zoom = 10.0;
//       if (maxDiff > 10) zoom = 2.0;
//       else if (maxDiff > 5) zoom = 4.0;
//       else if (maxDiff > 1) zoom = 6.0;
//       else if (maxDiff > 0.1) zoom = 8.0;
...
//   void showLocationBottomPanel(BuildContext context, MemoryCluster cluster, {List<Map<String, dynamic>>? specificMemories}) {
...
//           debugPrint('Memory clusters already loaded, fitting camera to clusters');
//           await _fitCameraToMemoryClusters();
//         } else {
...
//       final centerPoint = locations.isNotEmpty
//           ? locations[0]
...
//     if (mapController == null) {
//       debugPrint(
//         '🔄 _ensureCameraZoom - Skipping: mapController null',
//       );
//       return;
//     }
...
//     try {
//       debugPrint('[MapController][onMapCreated] Map created - implementing refined flow');
//       debugPrint('[MapController][onMapCreated] currentZoom.value: ${currentZoom.value}');
//       debugPrint('[MapController][onMapCreated] isShowingNewLocations.value: ${isShowingNewLocations.value}');

//       // Clean up any existing resources first
//       if (currentAnnotationManager != null) {
//         debugPrint('[MapController][onMapCreated] Cleaning up existing annotation manager');
//         currentAnnotationManager = null;
//       }
//       annotations.clear();
...
//       // Step 3: Map Loaded Listener - proceed normally when map loads successfully
//       debugPrint('✅ Map Loaded Listener: Map loaded successfully - proceeding normally');

//       debugPrint('[MapController][onMapCreated] Map loaded and initialized successfully');
//     } catch (e) {
//       debugPrint('❌ ERROR in onMapCreated: $e');
//       // Step 4: Handle errors - this will be caught by onMapLoadErrorListener
//       await _handleMapboxLoadError(e);
//     }
//   }

//   /// Step 3: Handle successful map loading
//   Future<void> _onMapLoadedSuccessfully() async {
//     try {
//       debugPrint('🗺 Map loaded successfully - initializing map features');
...
//       debugPrint('✅ Map features initialized successfully');
//     } catch (e) {
//       debugPrint('❌ Error initializing map features after successful load: $e');
//       // If initialization fails, treat as error
//       // await _handleMapboxLoadError(e);
//     }
//   }

//   /// Step 4: Handle Mapbox load errors
//   Future<void> _handleMapboxLoadError(dynamic error) async {
//     // try {
//       debugPrint('🔍 Handling Mapbox load error: $error');
...
//         debugPrint('🌐 Detected connectivity error - checking offline tiles');

//         // Check if offline tiles are available
//         final hasOfflineTiles = await _hasDownloadedTiles();
...
//     try {
//       debugPrint('🌐 Connectivity restored - hiding internet screen and reloading map');

//       // Hide internet required screen by setting to loading state
//       _setState(MapInitializationState.loadingMap);

//       // Reload map automatically
//       await automaticMapReloadAfterConnectivityRestore();

//       // Trigger tile downloading if needed
//       final hasOfflineTiles = await _hasDownloadedTiles();
//       if (!hasOfflineTiles) {
//         debugPrint('🗺 Starting tile download after connectivity restoration');
//         _startTileDownloadInBackground();
//       }

//     } catch (e) {
//       debugPrint('❌ Error handling connectivity restoration: $e');
//       _setState(MapInitializationState.error);
//     }
//   }

//   /// Start tile download in background
//   void _startTileDownloadInBackground() {
//     try {
//       final backgroundService = Get.find<BackgroundTileDownloadService>();
//       if (!backgroundService.isDownloading.value) {
//         debugPrint('🗺 Starting background tile download');
//         backgroundService.setAutoDownloadEnabled(true);
//       }
//     } catch (e) {
//       debugPrint('❌ Error starting background tile download: $e');
//     }
//   }
...
//     if (connectivityService.isMapboxConnectivityError(errorString)) {
//       debugPrint('🗺 Detected Mapbox connectivity error - notifying connectivity service');
...
//       if (!hasInternetForMapbox) {
//         debugPrint('🌐 No internet for Mapbox services - setting internetRequired state');
//         _setState(MapInitializationState.internetRequired);
//         return;
//       } else {
//         debugPrint('🌐 Internet available but Mapbox failed - retrying in 3 seconds');
...
//           if (connectivityService.isMapboxConnectivityError(retryError.toString())) {
//             debugPrint('🌐 Retry failed with connectivity error - setting internetRequired state');
//             _setState(MapInitializationState.internetRequired);
//             return;
//           }
//         }
...
//       'Cyan', 'Yellow', 'Brown', 'Blue Grey', 'Pink',
//       'Indigo', 'Teal', 'Deep Orange', 'Light Green', 'Lime',
//       'Amber', 'Deep Purple', 'Green Accent', 'Red Accent', 'Blue Accent'
//     ];
...
//     debugPrint('🎨 YEAR COLOR MAPPING - Complete List (Past 50 + Next 50 years)');
...
//       final colorIndex = getColorIndexForYear(year);
//       debugPrint('  $year: $colorName (Index: $colorIndex, Color: ${color.toString()})');
//     }
...
//     debugPrint('  $baseYear: $currentColorName (Index: $currentColorIndex, Color: ${currentColor.toString()}) ⭐');
...
//       final hasPermission = await _checkLocationPermission();
//       if (!hasPermission) {
//         debugPrint('🌍 LOCATION - No location permission, using default center');
...
//       debugPrint('🌍 LOCATION - User location set to: ${position.latitude}, ${position.longitude}');
//     } catch (e) {
...
//       onMapLoadedListener: (v){
//            _onMapLoadedSuccessfully();
//       },
//       onMapLoadErrorListener: (mapLoadingErrorEventData) async {
//         _handleMapboxLoadError(mapLoadingErrorEventData);
//         // Step 4: Mapbox Error Listener - if triggered, show the Internet Required Screen
//         // debugPrint('❌ Mapbox Error Listener triggered');
//         // debugPrint('❌ Error message: ${mapLoadingErrorEventData.message}');
//         // debugPrint('❌ Error type: ${mapLoadingErrorEventData.type}');
//         // debugPrint('❌ Error sourceId: ${mapLoadingErrorEventData.sourceId}');
//         // debugPrint('❌ Error tileId: ${mapLoadingErrorEventData.tileId}');

//         // try {
//         //   // Handle the error using the refined flow
//         //   final errorMessage = mapLoadingErrorEventData.message.isNotEmpty
//         //       ? mapLoadingErrorEventData.message
//         //       : 'Unknown Mapbox error';
//         //   await _handleMapboxLoadError(errorMessage);
//         // } catch (e) {
//         //   debugPrint('❌ Error in Mapbox error listener: $e');
//         //   _setState(MapInitializationState.error);
//         // }
//       },
//     );
...
//     debugPrint('[MapController][setupApp] Starting sequential initialization logic');
...
//           debugPrint('⚠️ ConnectivityService not initialized after waiting, checking manually');
...
//         final connectionType = connectivityService.connectionType.value;
//         debugPrint('🌐 Initial connectivity: hasInternet = $hasInternet, type = $connectionType');
...
//         debugPrint('🔐 Starting with permission check - internet status will be checked after permissions are granted');
//         _setState(MapInitializationState.checkingPermission);
...
//         debugPrint('🔐 Error occurred but still starting with permission check');
//         _setState(MapInitializationState.checkingPermission);
//       }
//     });
//   }
...
//         debugPrint('🌐 First time load without internet and no offline tiles - showing internet screen');
//       } else {
//         shouldShowInternetScreen.value = false;
//         needsInternetConnection.value = false;
//         isFirstTimeLoad.value = false;
//         debugPrint('🌐 Internet available or offline tiles present - proceeding normally');
//       }
...
//     debugPrint('[MapController][startMapInitializationSequence] Starting state-based map initialization sequence');
...
//       if (currentInitializationState.value == MapInitializationState.initial ||
//           currentInitializationState.value == MapInitializationState.checkingPermission) {
//         debugPrint('[MapController][startMapInitializationSequence] Starting from permission check');
//         _setState(MapInitializationState.checkingPermission);
//       } else if (currentInitializationState.value == MapInitializationState.internetRequired) {
//         debugPrint('[MapController][startMapInitializationSequence] Internet required - staying in current state');
//         // Stay in internetRequired state - user needs to resolve connectivity
//         return;
//       } else {
//         debugPrint('[MapController][startMapInitializationSequence] Continuing from current state: ${currentInitializationState.value}');
//       }

//       debugPrint('[MapController][startMapInitializationSequence] State-based initialization sequence started');

//     } catch (e) {
//       debugPrint('[MapController][startMapInitializationSequence] Error in initialization sequence: $e');
//       debugPrint('[MapController][startMapInitializationSequence] Error type: ${e.runtimeType}');
//       debugPrint('[MapController][startMapInitializationSequence] Stack trace: ${StackTrace.current}');
...
//     debugPrint('[MapController][_performDirectMapInitialization] Performing direct map initialization');

//     try {
//       // Step 1: Set initial map location (permission should already be checked by state machine)
//       debugPrint('[MapController][_performDirectMapInitialization] Step 1: Setting initial map location');
//       final permissionService = Get.find<PermissionService>();
...
//       debugPrint('[MapController][_performDirectMapInitialization] Step 2: Waiting for map to be ready');
//       await _waitForMapReady();

//       // Step 3: Load data from database
//       debugPrint('[MapController][_performDirectMapInitialization] Step 3: Loading data from database');
//       await loadMemoriesFromDatabase();

//       // Step 4: Create clusters and draw lines
//       debugPrint('[MapController][_performDirectMapInitialization] Step 4: Creating clusters and drawing lines');
//       await _initializeMemoryClustering();

//       // Step 5: Add click to zoom functionality
//       debugPrint('[MapController][_performDirectMapInitialization] Step 5: Setting up click to zoom');
//       _setupClickToZoom();

//       debugPrint('[MapController][_performDirectMapInitialization] Direct map initialization completed successfully');

//     } catch (e) {
//       debugPrint('[MapController][_performDirectMapInitialization] Error in direct initialization: $e');
...
//     debugPrint('[MapController][_setInitialMapLocation] Setting initial location, permission: $hasLocationPermission');

//     if (hasLocationPermission) {
//       debugPrint('[MapController][_setInitialMapLocation] Permission granted, getting current location');
//       try {
//         final position = await geolocator.Geolocator.getCurrentPosition();

//         debugPrint('[MapController][_setInitialMapLocation] Got current position: ${position.latitude}, ${position.longitude}');

//         // Set current location
//         final currentLocation = mapbox.Position(position.longitude, position.latitude);
//         locations.clear();
//         locations.add(currentLocation);

//         debugPrint('[MapController][_setInitialMapLocation] Set current location in locations list');

//       } catch (e) {
//         debugPrint('[MapController][_setInitialMapLocation] Error getting current location: $e');
//         debugPrint('[MapController][_setInitialMapLocation] Falling back to hardcoded location');
//         _setHardcodedLocation();
//       }
//     } else {
//       debugPrint('[MapController][_setInitialMapLocation] Permission denied, using hardcoded location');
//       _setHardcodedLocation();
//     }
//   }
...
//     debugPrint('[MapController][_setHardcodedLocation] Setting hardcoded location');
...
//     debugPrint('[MapController][_setHardcodedLocation] Set hardcoded location: $defaultLat, $defaultLng');
//   }
...
//       debugPrint('[MapController][_waitForMapReady] Check attempt ${attempts + 1}/$maxAttempts');
//       debugPrint('[MapController][_waitForMapReady] isMapReady: ${isMapReady.value}');
//       debugPrint('[MapController][_waitForMapReady] mapController null: ${mapController == null}');
...
//           debugPrint('[MapController][_waitForMapReady] Moving camera to initial location');
//           await mapController!.flyTo(
//             mapbox.CameraOptions(
//               center: mapbox.Point(coordinates: locations[0]),
//               zoom: 5.0,
...
//           );
//           debugPrint('[MapController][_waitForMapReady] Camera moved to initial location');
//         }
...
//     debugPrint('[MapController][_waitForMapReady] WARNING: Map not ready after ${maxAttempts * checkInterval.inMilliseconds}ms');
//   }

//   /// Setup click to zoom functionality
//   void _setupClickToZoom() {
//     debugPrint('[MapController][_setupClickToZoom] Setting up click to zoom functionality');

//     // Click to zoom is now handled by the _onMapTapped method
//     debugPrint('[MapController][_setupClickToZoom] Click to zoom setup completed');
//   }

//   /// Handle map tap events for click to zoom functionality
//   void _onMapTapped(mapbox.MapContentGestureContext context) {
//     debugPrint('[MapController][_onMapTapped] Map tapped at coordinates: ${context.point.coordinates}');

//     if (mapController == null) {
//       debugPrint('[MapController][_onMapTapped] Map controller is null, ignoring tap');
//       return;
//     }
...
//       debugPrint('[MapController][_onMapTapped] Current zoom level: $currentZoomLevel');
...
//         debugPrint('[MapController][_onMapTapped] Already at maximum zoom, ignoring tap');
//         return;
//       }
...
//       debugPrint('[MapController][_onMapTapped] Animating to tapped location with zoom: $newZoomLevel');
//       mapController!.flyTo(
//         mapbox.CameraOptions(
...
//       debugPrint('[MapController][_onMapTapped] Click to zoom animation started');
...
//     debugPrint('[MapController][printAllClusterData] ========== ALL CLUSTER DATA DUMP ==========');
//     debugPrint('[MapController][printAllClusterData] Total Clusters: ${currentClusters.length}');
//     debugPrint('[MapController][printAllClusterData] Total Arrows: ${currentArrows.length}');
//     debugPrint('[MapController][printAllClusterData] Cluster Level: ${currentClusterLevel.value}');
//     debugPrint('[MapController][printAllClusterData] Selected Cluster: ${selectedCluster.value?.id ?? 'None'}');

//     if (currentClusters.isEmpty) {
//       debugPrint('[MapController][printAllClusterData] ❌ NO CLUSTERS AVAILABLE');
//       return;
//     }
...
//       debugPrint('[MapController][printAllClusterData] === CLUSTER ${i + 1} OF ${currentClusters.length} ===');
//       debugPrint('[MapController][printAllClusterData] Cluster ID: ${cluster.id}');
//       debugPrint('[MapController][printAllClusterData] Memory Count: ${cluster.memoryCount}');
//       debugPrint('[MapController][printAllClusterData] Center: ${cluster.centerLatitude.toStringAsFixed(6)}, ${cluster.centerLongitude.toStringAsFixed(6)}');
//       debugPrint('[MapController][printAllClusterData] Radius: ${cluster.radiusKm}km');
//       debugPrint('[MapController][printAllClusterData] Date Range: ${cluster.earliestDate} to ${cluster.latestDate}');
//       debugPrint('[MapController][printAllClusterData] Is Single Memory: ${cluster.memoryCount == 1}');

//       // Print all memories in this cluster
//       debugPrint('[MapController][printAllClusterData] --- MEMORIES IN CLUSTER ${i + 1} ---');
//       for (int memIndex = 0; memIndex < cluster.memories.length; memIndex++) {
//         final memory = cluster.memories[memIndex];
//         debugPrint('[MapController][printAllClusterData] Memory ${memIndex + 1}/${cluster.memories.length}:');
//         debugPrint('[MapController][printAllClusterData]   ID: ${memory.id}');
//         debugPrint('[MapController][printAllClusterData]   Title: "${memory.title}"');
//         debugPrint('[MapController][printAllClusterData]   Description: "${memory.description}"');
//         debugPrint('[MapController][printAllClusterData]   Location: ${memory.latitude.toStringAsFixed(6)}, ${memory.longitude.toStringAsFixed(6)}');
//         debugPrint('[MapController][printAllClusterData]   Date: ${memory.memoryDate}');

//         // Print key metadata
//         final metadata = memory.metadata;
//         if (metadata.containsKey('location')) {
//           debugPrint('[MapController][printAllClusterData]   Original Location: "${metadata['location']}"');
//         }
//         if (metadata.containsKey('place_category')) {
//           debugPrint('[MapController][printAllClusterData]   Category: "${metadata['place_category']}"');
//         }
//         if (metadata.containsKey('hashtags')) {
//           debugPrint('[MapController][printAllClusterData]   Hashtags: "${metadata['hashtags']}"');
//         }

//         // Check for media
//         final hasImages = metadata.containsKey('images') && metadata['images'] != null && metadata['images'].toString().isNotEmpty;
//         final hasVoice = metadata.containsKey('voice_notes') && metadata['voice_notes'] != null && metadata['voice_notes'].toString().isNotEmpty;
//         debugPrint('[MapController][printAllClusterData]   Media: Images=${hasImages}, Voice=${hasVoice}');

//         // Print timestamps
//         if (metadata.containsKey('created_at')) {
//           debugPrint('[MapController][printAllClusterData]   Created: ${metadata['created_at']}');
//         }
//         if (metadata.containsKey('updated_at')) {
//           debugPrint('[MapController][printAllClusterData]   Updated: ${metadata['updated_at']}');
//         }
//       }
//     }

//     // Print arrow information
//     debugPrint('[MapController][printAllClusterData] === CHRONOLOGICAL ARROWS ===');
//     debugPrint('[MapController][printAllClusterData] Total Arrows: ${currentArrows.length}');

//     for (int i = 0; i < currentArrows.length; i++) {
//       final arrow = currentArrows[i];
//       debugPrint('[MapController][printAllClusterData] Arrow ${i + 1}:');
//       debugPrint('[MapController][printAllClusterData]   From: ${arrow.fromLatitude.toStringAsFixed(6)}, ${arrow.fromLongitude.toStringAsFixed(6)} (${arrow.fromClusterId})');
//       debugPrint('[MapController][printAllClusterData]   To: ${arrow.toLatitude.toStringAsFixed(6)}, ${arrow.toLongitude.toStringAsFixed(6)} (${arrow.toClusterId})');
//       debugPrint('[MapController][printAllClusterData]   Time: ${arrow.fromDate} → ${arrow.toDate}');
//       debugPrint('[MapController][printAllClusterData]   Distance: ${arrow.distance.toStringAsFixed(2)}km, Bearing: ${arrow.bearing.toStringAsFixed(1)}°');
//     }

//     debugPrint('[MapController][printAllClusterData] ========== END CLUSTER DATA DUMP ==========');
//   }

//   /// Print cluster statistics summary
//   void printClusterStats() {
//     debugPrint('[MapController][printClusterStats] === CLUSTER STATISTICS ===');
//     debugPrint('[MapController][printClusterStats] Total Memories: ${allMemories.length}');
//     debugPrint('[MapController][printClusterStats] Total Clusters: ${currentClusters.length}');
//     debugPrint('[MapController][printClusterStats] Total Arrows: ${currentArrows.length}');
//     debugPrint('[MapController][printClusterStats] Cluster Level: ${currentClusterLevel.value}');

//     if (currentClusters.isNotEmpty) {
//       final memoryCounts = currentClusters.map((c) => c.memoryCount).toList();
//       final totalMemoriesInClusters = memoryCounts.reduce((a, b) => a + b);
//       final avgMemoriesPerCluster = totalMemoriesInClusters / currentClusters.length;
//       final maxMemoriesInCluster = memoryCounts.reduce((a, b) => a > b ? a : b);
//       final minMemoriesInCluster = memoryCounts.reduce((a, b) => a < b ? a : b);

//       debugPrint('[MapController][printClusterStats] Memories in Clusters: $totalMemoriesInClusters');
//       debugPrint('[MapController][printClusterStats] Avg Memories/Cluster: ${avgMemoriesPerCluster.toStringAsFixed(1)}');
//       debugPrint('[MapController][printClusterStats] Max Memories in Single Cluster: $maxMemoriesInCluster');
//       debugPrint('[MapController][printClusterStats] Min Memories in Single Cluster: $minMemoriesInCluster');

//       final singleMemoryClusters = currentClusters.where((c) => c.memoryCount == 1).length;
//       debugPrint('[MapController][printClusterStats] Single Memory Clusters: $singleMemoryClusters');
//       debugPrint('[MapController][printClusterStats] Multi Memory Clusters: ${currentClusters.length - singleMemoryClusters}');
//     }
...
//     debugPrint('[MapController][_getUniqueColorForCluster] Generating unique color for cluster: $clusterId, memory: $memoryId');
...
//     debugPrint('[MapController][_getUniqueColorForCluster] Combined ID: $combinedId, Hash: $hash');
...
//     debugPrint('[MapController][_getUniqueColorForCluster] Selected color index: $colorIndex, Color: ${selectedColor.toString()}');
...
//     debugPrint('[MapController][verifyClusterIdUniqueness] === VERIFYING CLUSTER ID UNIQUENESS ===');
...
//       if (clusterIds.contains(cluster.id)) {
//         duplicateIds.add(cluster.id);
//         debugPrint('[MapController][verifyClusterIdUniqueness] ❌ DUPLICATE: Cluster ${i + 1} has duplicate ID: ${cluster.id}');
//       } else {
//         clusterIds.add(cluster.id);
//         debugPrint('[MapController][verifyClusterIdUniqueness] ✅ UNIQUE: Cluster ${i + 1} has unique ID: ${cluster.id}');
//       }
//     }

//     debugPrint('[MapController][verifyClusterIdUniqueness] === UNIQUENESS VERIFICATION RESULTS ===');
//     debugPrint('[MapController][verifyClusterIdUniqueness] Total clusters: ${currentClusters.length}');
//     debugPrint('[MapController][verifyClusterIdUniqueness] Unique IDs: ${clusterIds.length}');
//     debugPrint('[MapController][verifyClusterIdUniqueness] Duplicate IDs: ${duplicateIds.length}');

//     if (duplicateIds.isNotEmpty) {
//       debugPrint('[MapController][verifyClusterIdUniqueness] ❌ CRITICAL: Found ${duplicateIds.length} duplicate IDs: $duplicateIds');
//       return false;
//     } else {
//       debugPrint('[MapController][verifyClusterIdUniqueness] ✅ SUCCESS: All ${currentClusters.length} cluster IDs are unique');
//       return true;
//     }
//   }
...
//     debugPrint('[MapController][printClusterIdStats] === CLUSTER ID STATISTICS ===');
...
//     debugPrint('[MapController][printClusterIdStats] Total clusters: ${currentClusters.length}');
//     debugPrint('[MapController][printClusterIdStats] Unique IDs: ${idCounts.length}');
...
//         debugPrint('[MapController][printClusterIdStats]   ID "${duplicate.key}" appears ${duplicate.value} times');
//       }
//     } else {
//       debugPrint('[MapController][printClusterIdStats] ✅ All cluster IDs are unique');
//     }
...
//       debugPrint('[MapController][printClusterIdStats]   Cluster ${i + 1}: ${currentClusters[i].id}');
//     }

//     debugPrint('[MapController][printClusterIdStats] === END ID STATISTICS ===');
//   }
